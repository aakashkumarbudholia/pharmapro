...
public function changepassword($id)
 ...
 public function check_changepassword(Request $request)
  ...
  return view('front/multi-step-form',compact('userdata','jobdata','interviewid','interviewdata','interviewee_data','question_data','service'));
    }
	  ...
	  $hosted = $request->input('hosted');
		...
		// start code to save data in user table
		 ...
		 $reg_password1 = $request->input('password1');
			    ...
			    if(!empty($check->email)){
			      return Redirect::to('/formulaire')->with('error', 'Email Already Exist.');
			    }
